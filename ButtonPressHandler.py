...
class ButtonPressHandler:
    ...
    def __init__(self, mode_button: ConfigurableButtonElement,
                 ...
                 hold_time=0.7, combo_press_listeners=None):
    ...
    def _button_value_changed(self, value):
        ...
        if value == 127:
            ...
            log(f"ButtonPressHandler: PRESSED")
            for func in self._press_fn:
                if func: 
                    func()
            ...
            if self._is_combo:
                log(f"ButtonPressHandler: COMBO RELEASE")
                for func in self._combo_release_fn:
                    if func:
                        func()
                        if hasattr(func, '__name__'):
                            log(f"{func.__name__} called")
                ...
                log(f"ButtonPressHandler: HOLD RELEASE")
                for func in self._hold_release_fn:
                    if func:
                        func()
                        if hasattr(func, '__name__'):
                            log(f"{func.__name__} called")
                ...
                log(f"ButtonPressHandler: RELEASE")
                for func in self._release_fn:
                    if func:
                        func()
                        if hasattr(func, '__name__'):
                            log(f"{func.__name__} called")
    ...
    def _combo_press_value_changed(self, value):
        if value and self._mode_button.is_pressed():
            log(f"ButtonPressHandler: COMBO PRESS")
            self._cancel_hold_timer()
            self._is_held = False
            self._is_combo = True
            for func in self._combo_press_fn: 
                func() 
                if hasattr(func, '__name__'): 
                    log(f"{func.__name__} called")    
    ...
    def _hold(self):
        if self._mode_button.is_pressed():
            log(f"ButtonPressHandler: HOLD")
            self._is_held = True
            for func in self._hold_fn:
                if func:
                    func()
                    if hasattr(func, '__name__'):
                        log(f"{func.__name__} called")
